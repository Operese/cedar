...
func TestRestoreResolvConf(t *testing.T) {
	...
	workDir := filepath.Join("/tmp", "cedar-"+uuid.NewString())
...
func TestTarXattrs(t *testing.T) {
	...
	testDir, err := os.MkdirTemp("/tmp", "cedar-xattr-test")
	asserter.AssertErrNil(err, true)
	extractDir, err := os.MkdirTemp("/tmp", "cedar-xattr-test")
	asserter.AssertErrNil(err, true)
...
func TestPingXattrs(t *testing.T) {
	...
	testDir, err := os.MkdirTemp("/tmp", "cedar-ping-xattr-test")
	asserter.AssertErrNil(err, true)
	t.Cleanup(func() { os.RemoveAll(testDir) })
